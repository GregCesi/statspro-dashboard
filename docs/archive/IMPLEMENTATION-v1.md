...
- [x] Livrable 1 — Setup repo
- [x] Livrable 3 — Table de patch BDL auditée
- [x] Livrable 14 — `docs/SCHEMA.md`
...
- [x] Livrable 2 — Script `scripts/migrate.py`
- [x] Livrable 4 — `data/dashboard.json` produit
...
- [x] Livrable 5 — Refacto `index.html` chargement JSON externe
...
- [x] Livrable 6 — Filtres sidebar `poc_status` + `data_quality`
- [x] Livrable 7 — Colonnes tableau + pills colorées + affichage `validation_note` en détail
- [x] Livrable 8 — KPIs PoC dans la kpi-bar
...
- [x] Livrable 9 — Mode édition inline dans le tableau détail
- [x] Livrable 10 — Indicateur "N modifs non exportées" dans le header
- [x] Livrable 11 — Bouton "Exporter modifs en JSON patch"
- [x] Livrable 12 — Bouton "Réinitialiser les modifs locales"
...
- [x] Livrable 13 — Export markdown + prompt template
...
- [x] Livrable 15 — README opérationnel
...
**15/15 livrables terminés** (maj 2026-05-20). Reste uniquement le tag Git `v1.0`.

- Phases 1-7 complètes et fonctionnelles
- 163 lignes dans `data/dashboard.json` avec les 3 champs PoC
- 59 endpoints V1 taggés (58 validés + 1 validated_slow) au 2026-05-19
- Enum `poc_status` étendu avec `validated_slow` (affiché "validé (lent)", pill orange)
- Workflow complet : édition inline → localStorage → export patch → apply_patch.py → commit
